...
final_plot <- ggplot() +
  ...
  scale_fill_gradientn(
    ...
    name = "The source of the participants",
    na.value = NA,
    breaks = c(30, 200, 400, 600, 800, 1000, 1200),
    labels = c("30", "200", "400", "600", "800","1000", "1200"),
    guide = guide_colorbar(
      barwidth = unit(0.5, "cm"),
      barheight = unit(2, "cm"),
      frame.colour = "black",
  ...
  theme_void() +
  theme(
    text = element_text(family = "sans"),  # 使用通用字体避免错误
    legend.title = element_text(size = 10, face = "bold"),
    legend.text = element_text(size = 9),
    legend.position = c(0.01, 0.01),  # 保持数字位置
    legend.justification = c(0, 0),
    legend.box = "vertical",
    legend.box.just = "left",
    legend.spacing.y = unit(0.2, "cm"),
...
# 添加虚拟点用于创建missing data图例（不会在地图上显示）
  geom_point(data = dummy_data, 
             aes(x = x, y = y, shape = category), 
             size = 0,color = "darkgrey", fill = "darkgrey") +
  ...
  theme_void() +
  theme(
    text = element_text(family = "sans"),  # 可替换为 family = "Corbel" 如需特定字体
    legend.title = element_text(size = 10, face = "bold"),
    legend.text = element_text(size = 9),
    legend.position = c(0.01, 0.01),  # 图例位置（左下角）
    legend.justification = c(0, 0),
    legend.box = "vertical",  # 垂直堆叠图例项
    legend.box.just = "left",
    legend.spacing.y = unit(0.2, "cm"),
    legend.margin = margin(5, 5, 5, 5),
    plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm")
...
final_plot <- ggplot() +
  # 绘制有数据的国家
  geom_sf(data = filter(world, !is.na(value)), 
          aes(fill = value), color = "white", linewidth = 0.2) +
  ...
  # 绘制没有数据的国家
  geom_sf(data = filter(world, is.na(value)), 
          aes(fill = "Missing data"), color = "white", linewidth = 0.2) +
  
  # 离散图例用于 Missing data
  scale_fill_manual(
    name = NULL,
    values = c("Missing data" = "darkgrey"),
    guide = guide_legend(
...
final_plot <- ggplot() +
  ...
  theme(
    ...
    legend.spacing.y = unit(0.2, "cm"),
    legend.margin = margin(5, 5, 5, 5),
    legend.text.align = 0,  # ✅ 图例文字左对齐
    legend.title.align = 0  # ✅ 图例标题左对齐
  )
